...
- (void)setup
{
    ...
    */
    ...
    //右按钮
    self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithTitle:@"搜索" style:UIBarButtonItemStyleDone target:self action:@selector(justSearch)];
...
- (void)loadMemberDataWithPage:(NSInteger)page pageNum:(NSString*)pageNum keyWord:(NSString*)keyword{
    ...
    [[MyAPI sharedAPI] getSearchResultWithParameters:dicPara result:^(BOOL success, NSString *msg, NSArray *arrays) {
        if (success) {
            
            if (arrays.count == 0) {
                dispatch_async(dispatch_get_main_queue(), ^{
                    [self.searchResultController.tableView.mj_footer endRefreshingWithNoMoreData];
                });
                _page--;
            }
            [_searchArray addObjectsFromArray:arrays];
...
-(void)endRefresh{
    [self.searchResultController.tableView.mj_header endRefreshing];
    [self.searchResultController.tableView.mj_footer endRefreshing];
}
- (void)addRefresh{
    __weak typeof(self) weakSelf = self;
    self.searchResultController.tableView.mj_header = [MJRefreshNormalHeader headerWithRefreshingBlock:^{
        if (_searchArray.count > 0) {
            [_searchArray removeAllObjects];
        }
        _page = 1;
        [weakSelf loadMemberDataWithPage:_page pageNum:_pageNum keyWord:_searchWord];
    }];
    
    self.searchResultController.tableView.mj_footer = [MJRefreshAutoNormalFooter footerWithRefreshingBlock:^{
        _page ++;
        [weakSelf loadMemberDataWithPage:_page pageNum:_pageNum keyWord:_searchWord];
    }];
}
...
- (void)searchBarSearchButtonClicked:(UISearchBar *)searchBar
{
    ...
    switch (self.searchResultShowMode) {
        ...
        case PYSearchResultShowModeEmbed: // 内嵌
            ...
            //self.searchResultController.tableView.backgroundColor = [UIColor yellowColor];
            ...
            [self.searchResultController.tableView mas_makeConstraints:^(MASConstraintMaker *make) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (_searchArray.count > 0) {
        searchModel *searModel = [_searchArray objectAtIndex:indexPath.row];
        [tuijianCell.storeImg sd_setImageWithURL:[NSURL URLWithString:searModel.doorimg] placeholderImage:[UIImage imageNamed:DEFAULTSTOREIMAGE]];
        tuijianCell.storeNameLa.text = searModel.shopname;
        [tuijianCell.starV configWithStarLevel:searModel.avgScore.floatValue];
        tuijianCell.disCountLab.text = [NSString stringWithFormat:@"%@",searModel.discount];
        tuijianCell.addressLab.text = searModel.addr;
        tuijianCell.diatanceLab.hidden = YES;
    }
